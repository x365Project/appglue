...
export abstract class BaseDataControl extends XBaseControl {
  ...
  getDesignValidationIssues(): ValidationIssue[] {
    ...
    if (!this.valueName) {
      issues.push(
        new ValidationIssue("Value name is required", undefined, this.id)
      );
    } else {
  ...
  getRuntimeValidationIssues(): ValidationIssue[] {
    ...
    if (
      this.requiredOnAllOutcomes &&
      this.valueName &&
      !this.getFormContext()?.getFormDataValue(this.valueName)
    ) {
      issues.push(
        new ValidationIssue(this.requiredMessage, this.valueName, this.id)
      );
    }
